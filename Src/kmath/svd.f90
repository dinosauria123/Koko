...
620 RETURN
  ...
  SUBROUTINE dsvd_solve ( m, n, U, S, V, b, x )
    !
    !
    ! DSVD_SOLVE solves a linear system
    !
    !     A * x = b
    !
    ! with an m x n matrix A in the least-squares sense using the singular
    ...
    !    real U(M,N) singular value decomposition of A
    !    real S(N)
    !    real V(N,N)
    ...
    REAL (dp), INTENT(in)  :: U(m,m), S(n), V(n,n), b(m)
    ...
    x(1:n) = MATMUL ( TRANSPOSE( v(1:n,1:n) ), sub(1:n) )

  END SUBROUTINE dsvd_solve
