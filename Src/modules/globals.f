...
C     SET BY THE -n COMMAND-LINE FLAG. WHEN TRUE, KOKO WILL
C     ASSEMBLE GNUPLOT SCRIPT FILES (BLACK.GPL, DRAWMCMD.GPL,
C     PLOTCONT.GPL, ...) AS USUAL BUT WILL NOT LAUNCH THE
C     GNUPLOT VIEWER VIA shell_command. THE EMBEDDED GUI
C     (gui_py/mainwindow.py) RELIES ON THIS FLAG SO IT CAN
C     RENDER THE PLOT ITSELF WITHOUT THE NATIVE GNUPLOT
C     WINDOW ALSO POPPING UP.
          LOGICAL NOLAUNCH_GNUPLOT

C     SET BY THE -G COMMAND-LINE FLAG. WHEN TRUE, KOKO WILL
C     ASSEMBLE GNUPLOT SCRIPT FILES AS USUAL BUT WILL ALSO
C     RENDER THE PLOT TO A PNG FILE FOR THE EMBEDDED GUI.
C     THIS FLAG IS USED BY gui_py/mainwindow.py SO THAT THE
C     FORTRAN BACKEND PRODUCES THE PNG IMAGE DIRECTLY,
C     AVOIDING A SECOND GNUPLOT WINDOW.
          LOGICAL GENERATE_PLOT_PNG
