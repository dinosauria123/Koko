...
C
          INTEGER NX,NY,IX,IY,ORTAG,I,ENNL,ENN,NT1ANG,NT1SIZ,DFLAG,MYJK
C
...
C
     ...
     1        'EXPANDING SLIT ENERGY DISTRIBUTION VALUES DO NOT EXIST'
          ...
          END IF
          IF(ENNL.LT.10) THEN
              WRITE(OUTLYNE,*) 'DEBUG PLTRED: ENNL=',ENNL,
     1        ' GDTARP(1)=',GDTARP(1),' GDTARP(ENNL+1)=',GDTARP(ENNL+1)
              CALL SHOWIT(1)
              OUTLYNE=
...
C
          ...
          END DO
          ENN=ENNL
C     SAVE DATA TO RED.GPL
          DO I=1,ENN+1
              IX=INT(((AGDTARP(I)-LLIM)/(ULIM-LLIM))*6000.0+2000.0)
              IY=INT(((AGDTAR(I)-0.0)/(100.0-0.0))*4200.0+1400.0)
              CALL drawdatasave2(IX,IY,1,3,2000,8000,1400,5600)
          END DO
...
C
              ...
              END IF
C     FLUSH AND CLOSE GPL FILES
              CALL drawdatasave2(-1,-1,0,0,0,0,0,0)
              RETURN
      END
