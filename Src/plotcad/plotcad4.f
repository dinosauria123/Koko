...
C
          INTEGER COLPAS,XPOINT,YPOINT,IWAVCOL
C
...
C     CIRCULAR DETECTOR
     ...
     1            *1000.0D0)/SPDSSIVAL)+3500
                  ...
                  CALL MY_COLTYP(COLPAS)
                  IF(I.EQ.0)
     1            CALL MY_PLOT(XPOINT,YPOINT,0,0,3000,7000,1500,5500)
                  IF(I.GT.0)
...
C     SELECT A COLOR
              ...
              CALL MY_COLTYP(COLPAS)
C
C     ROUTE THE POINT TO THE PER-WAVELENGTH GNUPLOT TRACE FILE:
C     WAV1=black(0) WAV2=yellow(1) WAV3=magenta(2) WAV4=red(3)
C     WAV5=cyan(4); WAV6-10 CYCLE THROUGH THE SAME FIVE CHANNELS
              IWAVCOL=INT(DSPOT(16))-1
              IF(IWAVCOL.GE.5) IWAVCOL=MOD(IWAVCOL,5)
              IF(IWAVCOL.LT.0) IWAVCOL=0
C
C     MOVE TO THE POINT
              IF(INT(DSPOT(7)).EQ.0.AND.DSPOT(12).NE.0.0D0) THEN
                  CALL MY_PLOTC(-1,-1,0,IWAVCOL,3000,7000,1500,5500)
                  CALL MY_PLOTC(XPOINT-10,YPOINT,1,IWAVCOL,
     1            3000,7000,1500,5500)
                  CALL MY_PLOTC(XPOINT+10,YPOINT,1,IWAVCOL,
     1            3000,7000,1500,5500)
                  CALL MY_PLOTC(-1,-1,0,IWAVCOL,3000,7000,1500,5500)
                  CALL MY_PLOTC(XPOINT,YPOINT-10,1,IWAVCOL,
     1            3000,7000,1500,5500)
                  CALL MY_PLOTC(XPOINT,YPOINT+10,1,IWAVCOL,
     1            3000,7000,1500,5500)
              END IF
