...
| 複素瞳関数 (CAPFN) | IDD_CAPFN | `CAPFNNRD` `CAPFN` `CAPGRID` `WAMAP` `AMAP` `FITZERN` `LISTOPD` `LISTZERN` `LISTREPT` `CAPFNROT` `PLOT CAPFNOPD/CAPFNAPD` `PLOTCON CAPFNOPD/CAPFNAPD` `CAPFNOUT/IN/ADD/CLR` | **実装済み**（CapfnDialog: NRD(偶数強制)+モード(CAPFN/PERFECT/SILENT)+波長解析(CAPGRID/WAMAP/AMAP/FITZERN)+一覧(LISTOPD/LISTZERN/LISTREPT)+OPD/振幅プロット(CAPFNROT+PLOT CAPFNOPD/CAPFNAPD[,min,max])+**等高線プロット(Contour OPD / Contour intensity → PLOT NEW+PLOTCON CAPFNOPD,CAPFNAPD、gnuplot set contour/splot 描画)**+瞳ファイル操作(CAPFNOUT/IN/ADD/CLR)。KDP2 IDD_CAPFN/RAYS.INC 相当） |
| 歪曲/非点/像面湾曲 (DISAST) | IDD_DISAST | `FLDCV,<orient>,<factor>,<n>` `AST,<orient>,<factor>,<n>` `DIST,<orient>,<factor>,<n>` `FISHDIST,<orient>,<factor>,<n>` `PLTFLDCV` `PLTAST` `PLTDIST` `PLTFDIST` | **実装済み**（旧DisastDialogを3分割: FldcvDialog/AstDialog/DistDialog。Graphsメニューの Field Curvature(FLDCV).../Astigmatism(AST).../Distortion(DIST)... が各設定ダイアログを開き、OKで設定→描画。方位(0/90度)+フィールド点数(10-50)+プロットチェック。注意: factor(word#2)は必ず明示的に1を送る（空wordは0.0と解釈されDISTがラベルなしで描画される）。PLT*は引数なし形式のみ受理（word#3は拒否される）。KDP2 IDD_DISAST/RAYS.INC 相当） |
