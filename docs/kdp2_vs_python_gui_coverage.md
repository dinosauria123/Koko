...
| 点列図 (DOTF) | IDD_DOTF | `DOTF` `PLTDOTF` `DIFLEICA NO,n` | **実装済み** (SPACE I/O+FAR+DOTF+PLTDOTF,,1)。**DotfDialog に分割**: Graphs>Optical Transfer Function>Diffraction (DOTF).../Diffraction (DOTF, Leica)... が設定ダイアログを開き、OKで SPACE I/O+FAR/NEAR+DOTF+PLTDOTF[ LEICA],,1+DRAW |
| 幾何 OTF (GOTF) | IDD_GOTF | `GOTF` `PLTGOTF` `GEOLEICA NO,n` | **実装済み** (SPACE I/O+FAR+GOTF+PLTGOTF,1)。**GotfDialog に分割**: Graphs>Optical Transfer Function>Geometical (GOTF).../Geometical (GOTF, Leica)... が設定ダイアログを開き、OKで SPACE I/O+FAR/NEAR+GOTF+PLTGOTF[ LEICA],1+DRAW |
